...
- **No `label`/`color`** — follow `/use-hyperpanes` launch conventions: leave both unset so each pane auto-tints from its worktree's git project (frame color + the worktree-folder track name as the title), applied once claude's shell starts in `cwd`. Setting either *overrides* the tint and a `label` *suppresses* the auto-title — so **distinguish tracks by the auto-title + `meta.task`, never a hand-picked color.** (Tint mechanics live in `/use-hyperpanes`; fan-out stays styling-agnostic.)
